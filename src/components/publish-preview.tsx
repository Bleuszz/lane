...
export function PublishPreview({ draft, accounts, rules, onConfirm, onClose }: {
  ...
  useEffect(() => { dialog.current?.showModal(); void trackReview({data:{event:"destination_preview"}}).catch(() => undefined); }, []);
  ...
  return <dialog ref={dialog} onCancel={e => { if (busy) e.preventDefault(); else onClose(); }} className="photo-dialog w-[min(900px,94vw)] rounded-2xl border border-line bg-surface p-0 text-ink shadow-2xl" aria-labelledby="publish-preview-title">
    ...
    <div className="flex flex-wrap items-center justify-between gap-4 border-t border-line p-6"><p className="max-w-md text-xs text-muted">Queueing authorises Lane to publish to these accounts. Each destination uses one publishing action when processing starts.</p><Button disabled={busy || problems.length > 0 || (ebay && schema.isPending)} onClick={() => void confirm()}>{busy ? "Queueing…" : ebay && schema.isPending ? "Checking eBay…" : "Confirm and queue publish"}</Button></div>
  </dialog>;
}
