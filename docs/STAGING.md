...
## eBay order polling activation gate

Apply migration 0017 to the explicitly selected staging database before running this build. Keep `LANE_EBAY_ORDER_POLLING=false` until an authorized sandbox test. When enabled, the protected worker polls one eBay account/page before selecting up to three listing jobs. This can create stock-change/delist jobs: enabling it is not merely a read-only dashboard action. No scheduler or polling flag was enabled here.

Use an authorized sandbox seller with Fulfillment permission (the existing OAuth scope list includes `sell.fulfillment`). Reconcile starting inventory first: state initialization establishes the tracking baseline, and older orders/links are not automatically applied. Verify actual PAID/cancellation values, discounted line totals and the exact listing/line IDs. Test multiple pages, a lost response, repeated orders, manual entry with the same line ID, a sale while publishing, pauses/reconnection, partial cancellations/refunds and malformed pages. Check both local quantity and confirmed remote follow-ups. Unknown fees are not zero and a PAID API status does not prove accessible bank cash.

Activity shows polling configuration, each account's last completed page, pending pages/errors and the latest 20 observations needing review. Record a reviewed sale using its exact line reference; there is no bulk dismiss/replay or automatic historical backfill. Inspect retained events when resolving older observations. Never reset a cursor or delete a receipt simply to clear an error. Long gaps beyond 89 days require explicit reconciliation. Measure total worker duration and account fairness before setting cadence; one account/page per invocation does not promise instant detection at scale. Account-wide rate-limit coordination and provider pagination consistency remain release checks.
