...
3. Configure production authentication, secure cookies, strong token-encryption secret, exact OAuth redirects, durable PostgreSQL, backups and connection pooling. Fail closed when required secrets are absent. Host product photos at destination-accessible HTTPS URLs; user browser data URLs alone do not solve eBay publishing.
